...
import { Outlet, useNavigate, NavLink, useLocation } from "@umijs/max";
...
import { savedTaskSelection } from "@/pages/Tasks";
  ...
  return (
    <div className="m-0">
      ...
      <div className="pb-2">
        {item.children!.map((c) =>
          c.path ? (
            <NavLink key={c.label} to={c.path} className={({ isActive }) => `block py-2.5 px-5 pb-2 pl-10 text-white/70 no-underline text-[15px] hover:text-white hover:bg-white/10 ${isActive ? "!text-white !font-medium" : ""}`}>
            ...
            {navItems.map((item, i) =>
              item.path ? (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `block py-3 text-white/80 no-underline text-[16px] cursor-pointer transition-[color,background] duration-200 border-l-[3px] border-l-transparent hover:text-white hover:bg-white/[0.08] whitespace-nowrap ${
                      isActive ? "!text-white !font-semibold !bg-white/[0.12] !border-l-white" : ""
                    } ${collapsed ? "px-0 flex justify-center" : "pl-[17px] pr-5"}`
                  }
                  onClick={(e) => {
                    if (item.path === `${PREFIX}/data` && savedTaskSelection.length > 0) {
                      e.preventDefault();
                      navigate(`/data?ids=${savedTaskSelection.map(String).join(",")}`);
                    }
                  }}
                >
