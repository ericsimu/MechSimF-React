...
const Tasks: React.FC = () => {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          if (r.success) { message.success('删除成功'); loadTasks() }
  ...
  const columns: TableColumnsType<SimTask> = [
    ...
    {
      title: '参数变更', dataIndex: 'param_diff', key: 'param_diff',
      render: (_: unknown, record: SimTask) =>
        record.param_diff ? <span className="task-id-link" onClick={() => showDiff(record)}>查看</span> : '-',
    },
    { title: '创建时间', dataIndex: 'create_time', key: 'create_time', render: (t: string) => t ? new Date(t).toLocaleString() : '-' },
    {
      title: '操作', key: 'actions',
      render: (_: unknown, record: SimTask) => (
        <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
          <Button type="link" size="small" onClick={() => navigate(`/mechsim/data/${record.id}`)}>详情</Button>
  ...
  return (
    <div className="tasks-page">
      ...
      <Modal title="参数变更详情" open={diffOpen} onCancel={() => setDiffOpen(false)} width={700} footer={null}>
        {diffRows.length > 0 ? (
          <Table
            size="small"
            pagination={false}
